...
namespace Comic
{
    public class TurningPageManager : BaseBehaviour
    {
        private AllocationPool<TurnPage> m_pages;
        [SerializeField] private Transform m_poolContainer;
        [SerializeField] private GameObject m_turnPagePrefab;
        private PageManager m_manager = null;
        ...
        public void SetFrontTexture(Texture2D texture) => m_frontTexture = texture;
        public void SetBackTexture(Texture2D texture) => m_backTexture = texture;

        //public void RefreshRenderingSortOrder() => m_pages.SortElements(false);

        #region AllocationPool
        private void OnInitTurnPage(TurnPage page)
        {
            page.Init(m_manager.GetValidBounds());
        }
        #endregion AllocationPool
        ...
        {
            if (parameters.Length < 1 || parameters[0] is not PageManager)
            {
                Debug.LogWarning("Wrong parameters");
                return;
            }
            
            m_manager = (PageManager)parameters[0];

            m_pages = new AllocationPool<TurnPage>(m_turnPagePrefab, m_poolContainer, 10, SortOrderMethod.Sort_None, null, OnInitTurnPage);
        ...
        private void TurnAnimation(TurnPage page, bool is_next, bool play_shadow, float duration, Ease in_out_ease)
        {
            page.PlayRotationSequence(is_next, in_out_ease, duration);
        ...
        private void TurnAnimationError(TurnPage page, bool is_next, bool play_shadow, float duration, Ease in_ease, Ease out_ease)
        {
//            page.PlayPageErrorSequence(is_next, m_errorAngle, in_ease, out_ease, duration);
        ...
        public IEnumerator TurnMultiplePagesCoroutine(bool is_next, int pages_number, float duration)
        {
            ...
            //    var new_page = m_pages.AllocateElement(front_sprite, back_sprite);
        ...
        public IEnumerator TurnPageCoroutine(bool is_next, float duration)
        {
            var new_page = m_pages.AllocateElement(m_frontTexture, m_backTexture);

            TurnAnimation(new_page, is_next, true, duration, Ease.InOutQuad);

            yield return new WaitUntil(() => !m_pages.IsCompute());
        }
        public IEnumerator TurnPageErrorCoroutine(bool is_next, float duration)
        {
            var new_page = m_pages.AllocateElement(m_frontTexture, m_backTexture);
        ...
        public IEnumerator TurnCoverCoroutine(bool is_next, float duration)
        {
            var new_page = m_pages.AllocateElement(m_frontTexture, m_backTexture);
        ...
        #region Utility
        private void OnScreenshot(ComicScreenshot type, Texture2D texture)
        {
        ...
        private void SortPages(List<TurnPage> pages)
        {
